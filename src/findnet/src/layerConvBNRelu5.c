...
void layerConvBNRelu5(
  ...
  if(rt_core_id()==0){
    ...
    dory_dma_memcpy_3d_custom(
    l2_W, // ext
    (l1_buffer + 9032) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
    12096, // size: dimension of matrix of weight * bytes_per_weight
    ...
    64, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
    21, // length_2: how many 2_d copies we need -> the dimension of the tile in n_features direction
    64, // length_0: legnth of the 1_d copy, the length of tile in w direction
    1, // dir
    &dma_read_evt_W // copy
    );
  ...
  if(rt_core_id()==0){
    ...
    last_nof_load = (_i_nof_load+1 == 4) ? 1 : 0;
    last_nif_load = (_i_nif_load+1 == 1) ? 1 : 0;
    last_h_load = (_i_h_load+1 == 2) ? 1 : 0;
    last_w_load = (_i_w_load+1 == 2) ? 1 : 0;

    // compute double buffering offsets and update db state
    db_x = !db_state_x ? 3840 : 0;
    db_W = !db_state_W ? 12096 : 0;
    db_y = !db_state_y ? 672 : 0;
    exec_db_x = db_state_x ? 3840 : 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 12096 : 0;
    ...
    if(iter<4*1*2*2-1) {
      ...
      x_tile_size_h   = (last_h_load)   ? 5 : 6;
      x_tile_size_w   = (last_w_load)   ? 7 : 10;
      x_tile_size_byte = x_tile_size_nif*x_tile_size_h*x_tile_size_w*1;
      x_length_h_px = (last_h_load) ? 5 : 6;
      ...
      dory_dma_memcpy_3d_custom(
        dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 6, 10, 64, 14, 64,  2, 2,0, pad_offset_h, pad_offset_w, 0, 1), // extern
      ...
      y_tile_size_w   = (last_w_load)   ? 6 : 8;
      W_tile_size_nof = (last_nof_load) ? 1 : 21;
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
        dory_dma_memcpy_3d_custom(
          dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 21, 3*3, 64, 3*3, 64, 0,0,0,0,0,0, 1), // ext
          (l1_buffer + 9032) + db_W, // loc
    ...
    k = (int16_t *) (l1_buffer + 33236 + _i_nof_exec*42);
    lambda = (int32_t *) (l1_buffer + 33368 + _i_nof_exec*84);
    W = (char *) (l1_buffer + 9032 + exec_db_W);
    y = (char *) (l1_buffer + 7684 + db_y);
    x_tile_size_nif_exec = (last_nif_exec) ? 64 : 64;
    x_tile_size_h_exec   = (last_h_exec)   ? 5 : 6;
    x_tile_size_w_exec   = (last_w_exec)   ? 7 : 10;

    y_tile_size_nof = (last_nof_exec) ? 1 : 21;
    y_tile_size_h   = (last_h_exec)   ? 4 : 4;
    y_tile_size_w   = (last_w_exec)   ? 6 : 8;
    y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*1;
    y_length_h_px = (last_h_exec) ? 4 : 4;
    y_length_nof_byte = (last_nof_exec)   ? 1 : 21;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
...
if(rt_core_id()==0){
    // wait for DMA write
      ...
      dory_dma_memcpy_3d_custom(
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 4, 8, 21, 14, 64, 0, 0, 0, 0, 0, 0, 1), // ext
        (l1_buffer + 7684) + db_y, // loc
