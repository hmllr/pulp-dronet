...
void layerConvBNRelu0(
  ...
  if(rt_core_id()==0){
    ...
    l2_x, // ext
    (l1_buffer + 0) + 0, // loc
    2193, // size: dimension of the buffer
    108, // stride_1: stride for the 3d copy: if we have to copy on n_features axis, this is the stride to change from first 2D space to the next ones.
    1, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
    51,// length_2: how many 2_d copies we need -> the dimension of the tile in n_features direction
    ...
    dory_dma_memcpy_3d_custom(
    l2_W, // ext
    (l1_buffer + 35114) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  ...
  // last-tile flags
    ...
    int last_w_load = (3 == 1) ? 1 : 0;

    int iter;
  // tile loop nest
  for(iter=0; iter<1*1*2*3; iter++) {
  if(rt_core_id()==0){
    ...
    last_w_load = (_i_w_load+1 == 3) ? 1 : 0;

    // compute double buffering offsets and update db state
    db_x = !db_state_x ? 2193 : 0;
    db_W = !db_state_W ? 800 : 0;
    db_y = !db_state_y ? 15360 : 0;
    exec_db_x = db_state_x ? 2193 : 0;
    ...
    if(iter<1*1*2*3-1) {
      x_tile_size_nif = (last_nif_load) ? 1 : 1;
      x_tile_size_h   = (last_h_load)   ? 14 : 51;
      x_tile_size_w   = (last_w_load)   ? 30 : 43;
      x_tile_size_byte = x_tile_size_nif*x_tile_size_h*x_tile_size_w*1;
      x_length_h_px = (last_h_load) ? 14 : 51;
      x_length_nif_byte = (last_nif_load)   ? 1 : 1;
      ...
      dory_dma_memcpy_3d_custom(
        dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 51, 43, 1, 108, 1,  3, 3,0, pad_offset_h, pad_offset_w, 0, 1), // extern
      ...
      y_tile_size_h   = (last_h_load)   ? 6 : 24;
      y_tile_size_w   = (last_w_load)   ? 14 : 20;
      W_tile_size_nof = (last_nof_load) ? 32 : 32;
      W_tile_size_nif = (last_nif_load) ? 1 : 1;
      W_tile_size_byte = W_tile_size_nof*W_tile_size_nif*1*5*5;
      W_length_nif_byte = (last_nif_load) ? 1 : 1;

      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
        dory_dma_memcpy_3d_custom(
          dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 32, 5*5, 1, 5*5, 1, 0,0,0,0,0,0, 1), // ext
          (l1_buffer + 35114) + db_W, // loc
    ...
    k = (int16_t *) (l1_buffer + 36726 + _i_nof_exec*64);
    lambda = (int32_t *) (l1_buffer + 36794 + _i_nof_exec*128);
    W = (char *) (l1_buffer + 35114 + exec_db_W);
    y = (char *) (l1_buffer + 4390 + db_y);
    x_tile_size_nif_exec = (last_nif_exec) ? 1 : 1;
    x_tile_size_h_exec   = (last_h_exec)   ? 14 : 51;
    x_tile_size_w_exec   = (last_w_exec)   ? 30 : 43;

    y_tile_size_nof = (last_nof_exec) ? 32 : 32;
    y_tile_size_h   = (last_h_exec)   ? 6 : 24;
    y_tile_size_w   = (last_w_exec)   ? 14 : 20;
    y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*1;
    y_length_h_px = (last_h_exec) ? 6 : 24;
...
if(rt_core_id()==0){
    // wait for DMA write
      ...
      dory_dma_memcpy_3d_custom(
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 24, 20, 32, 54, 32, 0, 0, 0, 0, 0, 0, 1), // ext
        (l1_buffer + 4390) + db_y, // loc
